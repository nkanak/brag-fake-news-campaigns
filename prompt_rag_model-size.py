...
def process_files(model_name, test_folder, train_folder, num_shots, result_file, vectorizer, train_vectors, train_data):
    ...
    with tqdm(total=len(test_files), desc=f"Processing files (model={model_name})") as pbar:
...
def main(model_sizes, num_shots):
    ...
    for model_size in model_sizes:
        ...
        # Print metrics
        print(f"\nResults for {model_size} model with {num_shots}-shot RAG:")
    ...
    with open(f'metrics_all_models_rag_{num_shots}_shots.json', 'w') as f:
        json.dump(all_metrics, f, indent=2)

if __name__ == "__main__":
    model_sizes = ['1b', '3b', '27b']
    num_shots = 3
    main(model_sizes, num_shots)