...
def check_astroturfing(model_name, text, similar_examples, temperature):
...
def process_files(model_name, test_folder, train_folder, num_shots, result_file, temperature, vectorizer, train_vectors, train_data):
    results = load_results(result_file)
    
    # Process test files
    test_files = [f for f in os.listdir(test_folder) if f.endswith('.json')]
    
    with tqdm(total=len(test_files), desc=f"Processing files (temp={temperature})") as pbar:
        for filename in test_files:
            ...
            prediction = check_astroturfing(model_name, text, similar_examples, temperature)
...
def main(model_size, num_shots, temperatures):
    ...
    for temp in temperatures:
        result_file = f'result_{model_size}_rag_{num_shots}_shots_temp_{temp:.2f}_no_graph.json'
        
        results = process_files(model_name, test_folder, train_folder, num_shots, result_file, temp, vectorizer, train_vectors, train_data)
        ...
        # Print metrics
        print(f"\nResults for {num_shots}-shot RAG (temperature = {temp:.2f}):")
    ...
    with open(f'metrics_{model_size}_rag_{num_shots}_shots_all_temps.json', 'w') as f:
...
if __name__ == "__main__":
    ...
    main(model_size, num_shots, temperatures)